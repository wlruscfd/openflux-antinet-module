...
⚠ **Protect-адаптер канону lifecycle не принадлежит, но и в модуле его тоже нет** — раньше здесь
стояло «остаётся в модуле, форму диктует data-plane библиотека». Форм ДВЕ, и обе теперь канонные под
одними именами на обеих платформах: `dialControl` (форма `net.Dialer.Control`) и `protectFdFunc`
(форма `func(fd int32) bool`) — см. [`shared/protect`](../protect/README.md) (android) и
[`shared/offtun`](../offtun/README.md) (десктоп).
